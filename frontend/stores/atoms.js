...
export const moveTaskPositionInSameCategoryAtom = atom(null, (get, set, active, over) => {
  const categories = get(categoriesAtom);
  const categoryTasks = [
    ...categories.find((item) => item.id === active.data.current.category).tasks,
  ];
  const oldPosition = active.data.current.position;
  const newPosition = over.data.current.position;
  ...
  const newCategories = categories.map((item) =>
    item.id === active.data.current.category ? { ...item, tasks: newTasks } : item
  );
  set(categoriesAtom, newCategories);
});

export const moveTaskToADifferentCategoryAtom = atom(null, (get, set, active, over) => {
  const categories = get(categoriesAtom);
...
export const moveTaskAtTheTopOfACategoryAtom = atom(null, (get, set, active, over) => {
  const categories = get(categoriesAtom);
  const activeTask = categories
    .find((item) => item.id === active.data.current.category)
    .tasks.find((task) => task.id === active.id);
